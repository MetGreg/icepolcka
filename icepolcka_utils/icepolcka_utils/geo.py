...
This module contains all functions that are related to any kind of distance-,
height- or bearing- calculations.
...
def get_bin_altitude(r, theta, site_alt=0, r_e=6371000, k=4/3):
    ...
    Args:
        ...
        theta (numpy.ndarray or float): Elevation angles in degrees with 0° at
            horizontal and +90° pointing vertically upwards from the radar.
            Broadcastable to the shape of r.
        site_alt (float): Altitude in [m] above mean sea level of the
            referencing radar site.
        r_e (float): Earth's radius [m].
        k (float): Adjustment factor to account for the refractivity gradient
            that affects radar beam propagation. In principle this is
            wavelength- dependent. The default of 4/3 is a good approximation
            for most weather radar wavelengths.
    ...
    """
    h = (np.sqrt(r**2 + (k*r_e)**2 + 2*r*k*r_e*np.sin(np.radians(theta)))
         - k*r_e + site_alt)
    return h


def get_bin_distance(r, theta, site_alt=0, r_e=6371000, k=4/3):
    """Calculates the distance between range bin and site

    Calculates the great circle distance while taking the refractivity of the
    atmosphere into account. Function taken from wradlib.org
    ...
    .. math::
        ...
        s = k a \\arcsin\\left(
        \\frac{r \\cos\\theta}{k a + h(r, \\theta, a, k)}\\right)
    ...
    Args:
        ...
        theta (numpy.ndarray or float): Elevation angles in degrees with 0° at
            horizontal and +90° pointing vertically upwards from the radar.
            Broadcastable to the shape of r.
        site_alt (float): Site altitude [m] above mean sea level. Same shape
            as r.
        r_e (float): Earth's radius [m].
        k (float): Adjustment factor to account for the refractivity gradient
            that affects radar beam propagation. In principle this is
    ...
    """
    h = get_bin_altitude(r, theta, site_alt, r_e, k)
    s = k*r_e*np.arcsin((r*np.cos(np.radians(theta)))/(k*r_e + h))
    return s
...
def get_pos_from_dist(site_location, distance, azimuth, r_e=6371000):
    ...
    Given a site location, a distance along the surface and a compass bearing
    to a target, this method returns the geographical coordinates of this
    target.
    ...
    lat2 = math.asin(math.sin(lat1)*math.cos(distance/r_e) +
                     math.cos(lat1)*math.sin(distance/r_e)*math.cos(
        bearing))
    lon2 = lon1 + math.atan2(
        math.sin(bearing)*math.sin(distance/r_e)*math.cos(lat1),
...
def get_target_distance(radar, target, r_e=6371000):
    ...
    Args:
        radar (tuple or list): Longitude, Latitude of the radar. Must be
            given in decimal degrees.
        target (tuple or list): Longitude, Latitude of the target. Must be
            given in decimal degrees.
        r_e (float): Earth's radius [m].
    ...
    a = math.sin(d_lat/2) * math.sin(d_lat/2) + math.sin(d_lon/2) * math.sin(
        d_lon/2) * math.cos(lat_radar) * math.cos(lat_target)

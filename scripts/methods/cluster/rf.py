...
This script executes the actual radar filter script. It is designed to work within a slurm array
job. For each time step, one job with a specific index is executed. This script finds the
corresponding data file and time step over the index from a file where the input files are listed.

This script creates subdirectories for all variables and runs the radar filter script for each
variable separately.
...
def _prep_folder(workdir, idx, var):
    """Prepare job folder

    Prepares the job folder by making the subdirectories (if they don't exist) and copying the radar
    filter code and the parameter file to the directory.
...
def _set_params(sub_folder, date, crsim_file, var):
...
def _main():
    ...
    filename, filetime = _get_files(idx)
    workdir = os.getcwd()
    print("Execute RF for all variables")
    for var in VARIABLES:
        print(var)
        sub_folder = _prep_folder(workdir, idx, var)
        _set_params(sub_folder, filetime, filename, var)
        os.chdir(sub_folder)
        subprocess.run(["./radar_filter"], check=True)
